...
SERVER_VERSION = "cloud-app-crop-v21"
...
def _choose_name(mrz_name, printed_name, mrz_verified=False):
    """Reconcile MRZ name with a direct printed-value crop.

    Important: TD3 check digits DO NOT checksum the name zone. So an MRZ can be
    "verified" while filler '<' has been OCR'd as K/X/S inside the name.
    A clean crop of the printed name is therefore allowed to correct *safe*
    one-character MRZ noise even when other MRZ fields verify.
    """
    ...
    # Remove obvious padding words from the end of MRZ names, including noisy
    # forms such as KKKKKE / KXSKK that are not pure K/X/S.
    mw = m.split()
    ...
    if len(mt) != len(pt):
        # If printed crop simply removed a filler tail, accept it.
        if _edit(m_trim.replace(" ", ""), p.replace(" ", "")) <= 1:
            return p
        return m
    ...
    for idx, (a, b) in enumerate(zip(mt, pt)):
        ...
        # Never allow a crop to drop the first character of the FIRST name word:
        # KASHAF -> ASHAF is exactly the failure we want to prevent.
        if idx == 0 and len(a) == len(b) + 1 and a[1:] == b:
            return m

        # MRZ filler before later word: JAWAD KALI -> JAWAD ALI.
        if idx > 0 and len(a) == len(b) + 1 and a[0] in "KXS" and a[1:] == b:
            total_edits += 1
            continue

        # MRZ filler after word: TASSAWARK -> TASSAWAR.
    ...
    # A direct value crop may safely correct at most two small character errors.
    if total_edits <= 2:
        return p
...
async def read_passport_fields(
    ...
    result.update({
        "success": True,
        "mode": "dynamic_value_anchor_v20",
        "client_crop_mode": client_crop_mode or "dynamic_guaranteed_10_v21",
        "server_version": SERVER_VERSION,
        "ocr_time_ms": elapsed_ms,
