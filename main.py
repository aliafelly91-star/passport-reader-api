...
v4: the Flutter app detects labels/crops locally and sends only field crops.
"""
...
SERVER_VERSION = "cloud-app-crop-v4"
...
def _ocr_crop(data: bytes, kind: str):
    ...
    for v in _enhance(gray, 900 if kind != "sex" else 600):
        raw = _ocr(v, wl, 7)
        if raw and (not best or len(raw) < len(best)):
            best = raw
    return best
...
def _try_mrz(l1, l2):
    ...
    try:
        ...
        f = checker.fields()
        try:
            verified = checker.report.warnings == []
        except Exception:
            verified = False
        ...
        if raw in NATIONALITY_NAMES: raw = NATIONALITY_NAMES[raw]
            ...
            for code,country in COUNTRY_NAMES.items():
                if raw == country:
                    ...
                    break
        if raw and not any(w in LABEL_WORDS for w in raw.split()):
