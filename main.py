...
SERVER_VERSION = "cloud-app-crop-v18"
...
def _choose_name(mrz_name, printed_name, mrz_verified=False):
    """Reconcile a structural MRZ name with the printed crop conservatively.

    MRZ is the source of truth for given/surname because it has explicit
    separators. Printed OCR is allowed to correct only the classic trailing
    filler mistake (< read as K/X/S), e.g. TASSAWARK -> TASSAWAR. It is NOT
    allowed to delete a leading letter (KASHAF -> ASHAF) or replace a clean MRZ
    name with unrelated crop noise.
    """
    ...
    if len(mw) == len(pw):
        ...
        for a, b in zip(mw, pw):
            ...
            if a.startswith(b):
                extra = a[len(b):]
                if extra and len(extra) <= 4 and re.fullmatch(r"[KXS]+", extra):
                    changed = True
                    continue
            safe = False
            break
    ...
    # A full-name trailing filler tail is also safe to remove.
    cm = m.replace(" ", "")
    cp = p.replace(" ", "")
    if cm.startswith(cp):
        extra = cm[len(cp):]
