...
SERVER_VERSION = "cloud-app-crop-v11"
...
def _ocr_crop(data: bytes, kind: str):
    ...
    variants = _field_variants(gray, 900 if kind != "sex" else 600)
    if not variants:
        return ""
    plain, sharp, otsu, strong_otsu, medium_strong_otsu = variants

    if kind in {"given", "surname", "father"}:
        # Do not stop on the first merely-plausible token (e.g. "SI").
        # Read a few complementary passes and choose the cleanest candidate.
        reads = [
            _ocr(sharp, wl, psm=7, timeout=5),
            _ocr(medium_strong_otsu, wl, psm=11, timeout=5),
            _ocr(sharp, wl, psm=6, timeout=5),
            _ocr(plain, wl, psm=6, timeout=5),
        ]
        return _choose_ocr_name_candidate(reads, kind)

    if kind in {"birth", "issue", "expiry"}:
        # PSM 11 + strong local contrast is much better on faint green print.
        reads = [
            _ocr(plain, wl, psm=11, timeout=5),
            _ocr(sharp, wl, psm=7, timeout=5),
            _ocr(medium_strong_otsu, wl, psm=11, timeout=5),
            _ocr(strong_otsu, wl, psm=11, timeout=5),
        ]
        dated = []
        for raw in reads:
            if not raw:
                continue
            ds = _find_dates(_strip_label(raw, kind))
            if ds:
                dated.append((raw, ds[0]))
        if dated:
            counts = {}
            for _, d in dated:
                counts[d] = counts.get(d, 0) + 1
            winner = max(counts, key=lambda d: counts[d])
            # Return a clean normalized text so the common parser below reads
            # the voted date instead of a one-off OCR hallucination.
            return f"{winner.day:02d} {MONTHS[winner.month - 1]} {winner.year:04d}"
        return next((x for x in reads if x), "")
    ...
    if kind == "passport":
        ...
        for raw in reads:
            flat = re.sub(r"[^A-Z0-9]", "", (raw or "").upper())
            tokens = re.findall(r"[A-Z0-9]{7,11}", flat)
    ...
    if kind == "nationality":
        reads = [
            _ocr(sharp, wl, psm=7, timeout=4),
            _ocr(medium_strong_otsu, wl, psm=11, timeout=4),
            _ocr(strong_otsu, wl, psm=11, timeout=4),
        ]
        return max((x for x in reads if x), key=len, default="")

    # Sex is normally recovered from MRZ; keep this cheap.
    return (_ocr(sharp, wl, psm=7, timeout=4) or _ocr(medium_strong_otsu, wl, psm=11, timeout=4) or _ocr(strong_otsu, wl, psm=11, timeout=4))
...
def _mrz_from_bytes(data: bytes):
    ...
    strong = cv2.createCLAHE(clipLimit=4.0, tileGridSize=(8, 8)).apply(gray)
    ...
    for image_variant, psm in passes:
        ...
        # Early exit only when the strict checker verified a complete, clean read.
        if (
            best
            and best.get("is_verified")
            and best.get("passport_number")
            and best.get("birth_date")
            and best.get("expiry_date")
            and not _name_has_mrz_noise(best.get("surname_en", ""))
            and not _name_has_mrz_noise(best.get("given_name_en", ""))
        ):
            return best, debug
