...
def _choose_name(mrz_name, printed_name, mrz_verified=False):
    """Printed value crop is authoritative when it produced a plausible name.

    TD3 check digits do not validate the name area, so an MRZ can be "verified"
    while '<' filler is OCRed as K/X/S (SAMEERK, JAWAD KALI KKE...). The app
    deliberately sends a focused printed crop; when that crop is plausible it
    must win. MRZ is only the fallback when the crop failed.
    """
    p = _clean_name(printed_name)
    if _plausible_name(p):
        return p
    return _clean_name(mrz_name)
...
def _choose_passport_number(mrz_number, printed_number, country=""):
