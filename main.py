...
def _choose_name(mrz_name, printed_name, mrz_verified=False):
    """Prefer a plausible focused printed name over a near-identical MRZ read.

    MRZ check digits do not validate the name area. A filler '<' can become K
    while the MRZ still verifies, producing SAMEERK / TASSAWARK. Therefore a
    printed crop that differs by only a couple of characters wins.
    """
    ...
    if compact_m.startswith(compact_p):
        ...
        if 1 <= len(extra) <= 3 and all(ch in "KXS" for ch in extra):
            return p
...
def _batch_prepare_crop(data: bytes, kind: str, target_h: int = 92):
    gray = _decode(data)
    if gray is None:
        return None

    if kind in {"given", "surname", "father", "nationality", "sex"}:
    ...
    new_w = max(80, min(1500, int(w * scale)))
    return cv2.resize(gray, (new_w, target_h), interpolation=cv2.INTER_CUBIC)


def _batch_ocr_printed(data: dict):
    """Read all printed field crops with ONE Tesseract subprocess.

    This is the main stability fix for Render. The old path spawned Tesseract
    once for MRZ and again for father, issue date, names, etc. On a burstable
    CPU the first passport worked and later calls timed out. Here all printed
    fields share one OCR invocation, so a request normally uses at most two
    subprocesses total: printed batch + MRZ.
    """
    specs = [
    ...
    margin_x = 28
    margin_y = 18
    max_w = max(im.shape[1] for _, _, im in prepared)
    canvas = np.full(
        (slot_h * len(prepared), max_w + margin_x * 2), 255, dtype=np.uint8
    )
    bands = []

    for i, (field, kind, im) in enumerate(prepared):
        y0 = i * slot_h
        y = y0 + margin_y
        x = margin_x
        h, w = im.shape[:2]
        canvas[y:y + h, x:x + w] = im
        bands.append((field, kind, y0, y0 + slot_h))
    ...
    try:
        ...
        logger.warning("Printed batch OCR RuntimeError: %s", exc)
        return {}, ""
    except Exception as exc:
        logger.warning("Printed batch OCR failed (%s): %s", type(exc).__name__, exc)
        return {}, ""

    words_by_field = {field: [] for field, _, _, _ in bands}
    for i, text in enumerate(ocr.get("text", [])):
        token = (text or "").strip()
        if not token:
            continue
        try:
            cy = float(ocr["top"][i]) + float(ocr["height"][i]) / 2.0
        except Exception:
            continue
        for field, _kind, y0, y1 in bands:
            if y0 <= cy < y1:
                words_by_field[field].append(token)
                break

    raw = {
        field: re.sub(r"\s+", " ", " ".join(tokens)).strip().upper()
        for field, tokens in words_by_field.items()
    }
    debug_text = " | ".join(f"{k}={v}" for k, v in raw.items() if v)
    return raw, debug_text[:900]
...
def _best_printed_name(raw, kind, mrz_hint=""):
    ...
    if kind == "father":
        comma = re.search(r"\b([A-Z]{2,})\s*,\s*([A-Z]{2,}(?:\s+[A-Z]{2,}){0,3})\b", s)
        if comma:
            left = comma.group(1)
...
async def read_passport_fields(
    ...
    # Printed batch is fallback for MRZ structural fields.
    for key in ("passport_number", "birth_date", "expiry_date", "nationality", "sex"):
