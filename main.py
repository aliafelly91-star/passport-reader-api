...
SERVER_VERSION = "cloud-app-crop-v19"
...
def _choose_name(mrz_name, printed_name, mrz_verified=False):
    """Merge MRZ + focused printed crop without inventing name letters.

    Rules:
    - A verified clean MRZ stays authoritative.
    - A focused crop may remove obvious filler K/X/S noise.
    - On an unverified MRZ, a leading K/X/S can be removed from a NON-FIRST
      given-name word (JAWAD KALI -> JAWAD ALI), but never from the first word
      (so KASHAF is never changed to ASHAF).
    - Unrelated printed OCR garbage never replaces a plausible MRZ name.
    """
    ...
    # Remove standalone filler tails from the MRZ comparison only.
    mw = m.split()
    while len(mw) > 1 and re.fullmatch(r"[KXS]{2,}", mw[-1] or ""):
        mw.pop()
    m_trim = " ".join(mw)

    if mrz_verified and not _name_has_mrz_noise(m):
        # Even verified MRZ can have one trailing filler char in the name zone,
        # but no broader edits are accepted.
        if m_trim == p:
            return p

    mt_words = m_trim.split()
    p_words = p.split()

    if len(mt_words) == len(p_words):
        safe = True
        changed = False

        for idx, (a, b) in enumerate(zip(mt_words, p_words)):
            if a == b:
                continue

            # Classic trailing filler: TASSAWARK -> TASSAWAR.
            if len(a) == len(b) + 1 and a[:-1] == b and a[-1] in "KXS":
                changed = True
                continue

            # Filler '<' read as K/X/S before a later given-name word:
            # JAWAD KALI -> JAWAD ALI. Never apply to word zero.
            if (
                not mrz_verified
                and idx > 0
                and len(a) == len(b) + 1
    ...
    # If the MRZ itself visibly contains filler noise and the focused crop is
    # plausible, prefer the crop only when it is still close to the trimmed MRZ.
    if not mrz_verified and _name_has_mrz_noise(m):
        if _edit(m_trim.replace(" ", ""), p.replace(" ", "")) <= 2:
            return p
...
def _choose_passport_number(mrz_number, printed_number, country="", mrz_verified=False):
    ...
    if country_up == "PAKISTAN":
        ...
        if p_ok and m_ok:
            ...
            if p == m:
                return m
            return p if _edit(p, m) <= 1 and not mrz_verified else m
...
async def read_passport_fields(
    ...
    if not any(useful):
        return JSONResponse(status_code=422, content={
            ...
            "mode": "dynamic_field_crops_batch",
            "server_version": SERVER_VERSION,
    ...
    result.update({
        "success": True,
        "mode": "dynamic_field_crops_batch",
        "client_crop_mode": client_crop_mode or "dynamic_label_value_v19",
        "server_version": SERVER_VERSION,
        "ocr_time_ms": elapsed_ms,
