...
SERVER_VERSION = "cloud-app-crop-v17"
...
def _choose_name(mrz_name, printed_name, mrz_verified=False):
    """Safely reconcile MRZ and the focused printed crop.

    A printed crop is useful for correcting tiny MRZ filler mistakes such as
    TASSAWARK -> TASSAWAR, but it must NOT overwrite a clean MRZ name with an
    unrelated OCR sentence (for example JAWAD ALI -> JAVVAD ALI RATORARTY).
    """
    ...
    if not m_ok:
        return p if p_ok else m
    if not p_ok:
        return m

    # If MRZ itself clearly contains filler noise, the focused printed value is
    # safer as long as it is plausible.
    if _name_has_mrz_noise(m):
        return p

    # When both are plausible, only let printed OCR replace a clean MRZ value
    # when they substantially agree. This keeps real corrections (SAMEERK ->
    # SAMEER, TASSAWARK -> TASSAWAR) and rejects crops containing neighbouring
    # labels/fields.
    distance = _edit(m, p)
    tolerance = 2 if max(len(m), len(p)) < 14 else 3
    if distance <= tolerance:
        return p

    compact_m = m.replace(" ", "")
    compact_p = p.replace(" ", "")
    if compact_m.startswith(compact_p) and len(compact_m) - len(compact_p) <= 2:
        return p
    if compact_p.startswith(compact_m) and len(compact_p) - len(compact_m) <= 2:
        return p

    # A clean MRZ name is the structural fallback when the crop disagrees by a
    # lot. Verified MRZ is especially strong, but this guard is useful even
    # when only the line structure was recovered.
    return m
...
def _batch_prepare_crop(data: bytes, kind: str, target_h: int = 118):
    ...
    for field, line_map in lines_by_field.items():
        ...
        if field == "father_name_crop":
            def father_line_score(row):
                ...
                label_penalty = 20 * sum(1 for w in words if w in LABEL_WORDS)
                comma_bonus = 18 if "," in line_text else 0
                # Names normally have 2-4 alphabetic words and high OCR confidence.
                count_bonus = 15 if 2 <= len(words) <= 4 else 0
                return avg_conf + alpha_len * 4 + comma_bonus + count_bonus - label_penalty
...
def _best_printed_name(raw, kind, mrz_hint=""):
    ...
    label_patterns = {
        "given": r"\bGIVEN\s+[A-Z]{2,8}\b",
        "surname": r"\bSUR[A-Z]{2,8}\b",
    }
...
def _choose_passport_number(mrz_number, printed_number, country="", mrz_verified=False):
    """Reconcile the MRZ number with the focused printed-number crop.

    Pakistani passports in this workflow use two letters + seven digits. A
    focused printed crop should correct one-character MRZ OCR mistakes such as
    GZ... -> 6Z..., but a crop that lost the two letters (101012622) must not
    overwrite a clean MRZ value.
    """
    ...
    if country_up == "PAKISTAN":
        ...
        # The MRZ document-number check digit validates this field. When MRZ is
        # verified and has the expected Pakistani format, do not replace it
        # with a visual OCR confusion such as GZ... -> 6Z....
        ...
        if m_ok and not p_ok:
            return m
        if p_ok and m_ok:
            return p if _edit(p, m) <= 1 else m

    if p and m:
        if len(p) == len(m) and _edit(p, m) <= 2:
            return p
        # Prefer the value that looks more like a passport number: contains
        # both letters and digits, with 7-10 characters total.
        def score(v):
            return (
                int(7 <= len(v) <= 10) * 3
...
async def read_passport_fields(
    ...
    # Printed names win when plausible. This is what prevents SAMEERK/TASSAWARK.
    for key in ("given_name_en", "surname_en"):
        p = printed.get(key, "")
        if p:
    ...
    if printed.get("father_name_en"):
        result["father_name_en"] = printed["father_name_en"]
        src["father_name_en"] = "printed_batch"
    if printed.get("issue_date"):
        result["issue_date"] = printed["issue_date"]
